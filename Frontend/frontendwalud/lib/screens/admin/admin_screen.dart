...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  // ✅ FIX: guardamos nombre y email por separado para no depender del objeto User
  //    que al refrescar puede volver con tipo_usuario = 'paciente' si el token
  //    no incluye el campo correctamente.
  ...
  Future<void> _loadUser() async {
    ...
    if (mounted && r['success']) {
      final u = r['user'] as User;
      setState(() {
        _adminName = u.fullName;
        _adminEmail = u.email;
      });
    }
  }
  ...
  static const _navItems = [
    ...
    _NavItem(icon: Icons.history_outlined, label: 'Historial Clínico'),
  ...
  Widget _buildScreen(int idx) {
    switch (idx) {
      ...
      case 4:
        return const AdminMedicalHistoryScreen();
      case 5:
  ...
  Widget _buildSidebarContent() {
    return Column(
      ...
      children: [
        ...
        // ✅ FIX ROL: mostramos siempre 'ADMINISTRADOR' hardcodeado.
        //    El label de rol NO viene del objeto User (que puede refrescar como 'paciente')
        //    sino que es fijo porque este widget solo se muestra a admins.
        InkWell(
          onTap: () {},
          child: Padding(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      // ✅ Siempre 'ADMINISTRADOR', no depende de tipo_usuario del User
                      const Text(
                        'ADMINISTRADOR',
                        style: TextStyle(
                          fontSize: 9,
...
// ══════════════════════════════════════════════════════════════════════════════
// HOME TAB — ✅ agrega stat de historias clínicas
// ══════════════════════════════════════════════════════════════════════════════
...
class _AdminHomeTabState extends State<_AdminHomeTab> {
  ...
  Future<void> _load() async {
    // ✅ Una sola llamada: el backend ya incluye total_historias en stats
    ...
    if (mounted)
      setState(() {
        ...
        if (r['success']) {
          _stats = r['data'];
          // Lee total_historias del endpoint de stats (campo agregado en AdminController)
          _totalHistorias = (_stats['total_historias'] ?? 0) as int;
        }
      });
  }
            ...
            if (_loading)
              ...
              GridView.count(
                ...
                children: [
                  ...
                  // ✅ NUEVO: total historias clínicas
                  _StatCard(
                    icon: Icons.history_outlined,
                    iconBg: const Color(0xFFF3E8FF),
                    iconColor: const Color(0xFF7C3AED),
                    label: 'Historias Clínicas',
                    value: '$_totalHistorias',
                    trend: '',
                    trendUp: true,
                  ),
                  _StatCard(
                  ...
                  _StatCard(
                    ...
                    value: '${_stats['citas_realizadas'] ?? 0}',
                    trend: '',
                    trendUp: true,
                  ),
                  _StatCard(
                    ...
                    value:
                        '\$${NumberFormat('#,##0').format((_stats['ingresos_totales'] ?? 0) is String ? double.tryParse(_stats['ingresos_totales'].toString()) ?? 0 : _stats['ingresos_totales'])}',
                    trend: '',
                    trendUp: true,
                  ),
                ],
            ...
            Row(
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(
                    isMedicoTab ? 'Nuevo Médico' : 'Crear Usuario',
...
class _AdminMedicalHistoryScreenState extends State<AdminMedicalHistoryScreen> {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Accede y supervisa todas las evoluciones médicas registradas.',
              style: TextStyle(color: Colors.grey[500], fontSize: 13),
            ),
            const SizedBox(height: 20),
                                    ...
                                    children: [
                                      ...
                                      SizedBox(
                                        ...
                                        child: Container(
                                          ...
                                          child: Text(
                                            especialidadLabel(rec.especialidad),
                                            style: const TextStyle(
                                              color: Color(0xFF4F46E5),
                                              fontSize: 11,
                                              fontWeight: FontWeight.bold,
              ...
              children: [
                ...
                Padding(
                  ...
                  child: SingleChildScrollView(
                    scrollDirection: Axis.horizontal,
                    child: Row(
                      children: ['todas', 'pendiente', 'realizada', 'cancelada']
                          .map((f) {
    ...
    if (mounted)
      setState(() {
        ...
        if (statsR['success']) {
          ...
          _stats = {
            'total_completado':
                double.tryParse(raw['total_completado']?.toString() ?? '0') ??
                0.0,
            'total_pendiente':
                double.tryParse(raw['total_pendiente']?.toString() ?? '0') ??
                0.0,
            'total_cancelado':
                double.tryParse(raw['total_cancelado']?.toString() ?? '0') ??
                0.0,
          };
        }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Monitorea pagos, detecta fallos y verifica transacciones.',
            style: TextStyle(color: Colors.grey[500], fontSize: 13),
          ),
          const SizedBox(height: 20),

...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Widget _buildScreen(int idx) {
    switch (idx) {
      ...
      case 4: return const AdminPaymentsScreen();
      default: return const SizedBox();
    }
  }

  // Contenido del sidebar reutilizado en fijo y en Drawer
            ...
            itemBuilder: (_, i) {
              ...
              return GestureDetector(
                onTap: () {
                  ...
                  // Cierra el Drawer si está abierto en pantalla pequeña
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // En pantallas pequeñas el sidebar se convierte en Drawer
      drawer: isNarrow
          ? Drawer(width: _kSidebarWidth, child: _buildSidebarContent())
          : null,
      // AppBar solo en pantallas pequeñas para mostrar el botón hamburguesa
      appBar: isNarrow
          ? AppBar(
      ...
      body: Row(
        children: [
          // Sidebar fijo solo en pantallas anchas
          if (!isNarrow)
            Container(
...
class _AdminHomeTabState extends State<_AdminHomeTab> {
  ...
  @override
  Widget build(BuildContext context) {
    // Grid adaptable: 2 columnas en pantallas pequeñas, 4 en grandes
    final isNarrow   = MediaQuery.of(context).size.width < _kBreakpoint;
...
class _AdminUsersScreenState extends State<AdminUsersScreen> {
                ...
                if (!isMedicoTab)
                  Padding(
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Row(
                        children: ['Todos', 'Médicos', 'Pacientes', 'Admin'].map((f) {
                    ...
                    child: Column(children: [
                      Padding(
                        ...
                        child: Row(children: [
                          ...
                          if (isMedicoTab)
                            const SizedBox(width: 150, child: _TH('ESPECIALIDAD')),
                      ...
                      if (_loading)
                        ...
                        Padding(
                          ...
                          child: Center(child: Column(children: [
                            ...
                            const SizedBox(height: 12),
                            Text('No hay usuarios', style: TextStyle(
  ...
  // FORMULARIO CREAR / EDITAR — sin cambios
  void _showUserFormDialog(User? userToEdit) {
    ...
    String selectedRol = userToEdit?.isDoctor == true
        ? 'medico' : userToEdit?.isAdmin == true ? 'admin' : 'paciente';
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (selectedRol == 'medico') ...[
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    decoration: _fDeco('Selecciona especialidad',
                ...
                InkWell(
                  onTap: () async {
                    ...
                    if (d != null)
                      setS(() => birthCtrl.text = DateFormat('yyyy-MM-dd').format(d));
                  },
                  child: AbsorbPointer(
              ...
              onPressed: () async {
                ...
                if (isEdit) {
                  ...
                  final currentRol = userToEdit.isDoctor
                      ? 'medico' : userToEdit.isAdmin ? 'admin' : 'paciente';
                  if (selectedRol != currentRol) {
                    await AdminService.assignRole(userToEdit.id!, selectedRol,
                        especialidad: selectedEspecialidad);
                  }
                } else {
  ...
  Future<void> _confirmDelete(User user) async {
    ...
    if (confirm == true) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(r['success'] == true ? 'Usuario eliminado' : 'Error al eliminar'),
          backgroundColor: r['success'] == true ? const Color(0xFF10B981) : Colors.red));
        if (r['success'] == true) _load();
      }
    }
  ...
  void _showRolDialog(User user) {
    String selectedRol =
        user.isDoctor ? 'medico' : (user.isAdmin ? 'admin' : 'paciente');
      ...
      builder: (_) => AlertDialog(
        ...
        content: StatefulBuilder(
          builder: (ctx, setS) => Column(mainAxisSize: MainAxisSize.min, children: [
            ...['paciente', 'medico', 'admin'].map((r) => RadioListTile<String>(
              value: r, groupValue: selectedRol,
              activeColor: const Color(0xFF4F46E5),
              onChanged: (v) => setS(() => selectedRol = v!),
              title: Text(r == 'paciente' ? 'Paciente'
                  : r == 'medico' ? 'Médico' : 'Administrador'),
            )),
            if (selectedRol == 'medico') ...[
              const Divider(),
              DropdownButtonFormField<String>(
                value: selectedEspecialidad,
                hint: const Text('Especialidad médica',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final r = await AdminService.assignRole(user.id!, selectedRol,
                  especialidad: selectedEspecialidad);
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  content: Text(r['success'] == true
                      ? 'Rol asignado correctamente' : (r['message'] ?? 'Error')),
...
// Fila de usuario con anchos fijos para compatibilidad con scroll horizontal
class _UserRow extends StatelessWidget {
  final User user;
  final bool shaded, showEspecialidad;
  final VoidCallback onEdit, onRol, onToggleActive, onDelete;
  const _UserRow({
    required this.user, required this.shaded,
    this.showEspecialidad = false,
    required this.onEdit, required this.onRol,
    ...
    return Container(
      ...
      child: Row(children: [
        SizedBox(width: 220, child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(user.fullName, style: const TextStyle(
              fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF1A1A7A)),
              overflow: TextOverflow.ellipsis),
        ...
        SizedBox(width: 110, child: Container(
          ...
          decoration: BoxDecoration(
            color: rolColor.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
        ...
        SizedBox(width: 130, child: Row(children: [
          ...
          _ActionBtn(icon: Icons.manage_accounts_outlined,
              color: const Color(0xFF4F46E5), onTap: onRol, tooltip: 'Cambiar rol'),
          const SizedBox(width: 4),
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: SingleChildScrollView(
                scrollDirection: Axis.horizontal,
                child: Row(
                  children: ['todas', 'pendiente', 'realizada', 'cancelada'].map((f) {
    ...
    if (mounted)
      setState(() {
        ...
        if (statsR['success']) {
          ...
          _stats = {
            'total_completado': double.tryParse(
                raw['total_completado']?.toString() ?? '0') ?? 0.0,
            'total_pendiente':  double.tryParse(
                raw['total_pendiente']?.toString()  ?? '0') ?? 0.0,
            'total_cancelado':  double.tryParse(
                raw['total_cancelado']?.toString()  ?? '0') ?? 0.0,
            'count_completado': raw['count_completado'] ?? 0,
            'count_pendiente':  raw['count_pendiente']  ?? 0,
            'count_cancelado':  raw['count_cancelado']  ?? 0,
          };
        }
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Reportes de Pago', style: TextStyle(
          fontSize: 28, fontWeight: FontWeight.w900, color: Color(0xFF1A1A7A))),
        Text('Monitorea pagos, detecta fallos y verifica estados de transacciones.',
          style: TextStyle(color: Colors.grey[500], fontSize: 13)),
        const SizedBox(height: 24),
        ...
        if (_loading)
          ...
          // Cards: columna en pantalla pequeña, row en grande
          if (isNarrow)
            Column(children: [

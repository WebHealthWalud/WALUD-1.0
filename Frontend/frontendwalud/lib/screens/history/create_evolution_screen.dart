// lib/screens/history/create_evolution_screen.dart
...
class _CreateEvolutionScreenState extends State<CreateEvolutionScreen> {
  ...
  Future<void> _submit() async {
    if (_patientId == null) { _snack('Selecciona un paciente', Colors.red); return; }
    if (_motivoCtrl.text.trim().isEmpty) {
      _snack('El motivo de consulta es obligatorio', Colors.red); return;
    }
    if (_selectedEspecialidad == null) {
      _snack('Selecciona la especialidad', Colors.red); return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Nueva Evolución Médica',
  ...
  Widget _buildWide() => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        child: Theme(
          ...
          child: ExpansionTile(
            title: Row(children: const [
              ...
              SizedBox(width: 8),
              Text('Línea de Tiempo Clínica', style: TextStyle(
                  fontWeight: FontWeight.bold, color: Color(0xFF1A1A7A), fontSize: 14)),
            ]),
            children: [
              ConstrainedBox(
  ...
  Widget _buildForm({bool narrow = false}) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Nueva Evolución Médica', style: TextStyle(
              ...
              Container(
                ...
                child: Text('ID: #$_evolId', style: const TextStyle(
                    color: Color(0xFF4F46E5), fontWeight: FontWeight.bold,
                    fontSize: 12)),
              ),
            ]),
            const SizedBox(height: 4),
            Text('Busque al paciente para iniciar el registro.',
        ...
        // ── Especialidad
        _section(icon: Icons.medical_services_outlined, title: 'Especialidad',
          ...
          onChanged: (v) => setState(() => _selectedEspecialidad = v),
        )),
        const SizedBox(height: 14),

        // ── Motivo de consulta
        _section(icon: Icons.chat_bubble_outline, title: 'Motivo de Consulta',
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _label('Escriba el motivo principal de la visita *'),
          const SizedBox(height: 8),
          _tf(_motivoCtrl, 'Escriba el motivo principal de la visita...', 4),
        ])),
        const SizedBox(height: 14),

        // ── Examen Físico
        _section(icon: Icons.health_and_safety_outlined, title: 'Examen Físico',
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _label('Hallazgos relevantes del examen físico'),
          const SizedBox(height: 8),
          _tf(_examenCtrl, 'Hallazgos relevantes del examen físico...', 4),
        ])),
        const SizedBox(height: 14),

        // ── Diagnóstico
        _section(icon: Icons.biotech_outlined, title: 'Diagnóstico (CIE-10)',
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          narrow
              ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _label('Código CIE-10'), const SizedBox(height: 6),
                  _tf(_cie10Ctrl, 'Ej: J06.9', 1),
                  const SizedBox(height: 10),
                  _label('Nombre del diagnóstico'), const SizedBox(height: 6),
                  _tf(_diagNombreCtrl, 'Ej: Infección respiratoria aguda', 1),
                ])
          ...
          const SizedBox(height: 10),
          _label('Descripción del diagnóstico'), const SizedBox(height: 6),
          _tf(_diagDescCtrl, 'Describe el diagnóstico con más detalle...', 3),
        ])),
        const SizedBox(height: 14),

        // ── Tratamiento y Receta
        _section(icon: Icons.medication_outlined, title: 'Tratamiento y Receta',
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _label('Medicamentos e indicaciones (una línea por medicamento)'),
          const SizedBox(height: 6),
          _tf(_tratamientoCtrl,
              'Ej: Amoxicilina 500mg cada 8 horas por 7 días\n'
              'Ibuprofeno 400mg cada 6 horas si hay dolor...', 5),
          const SizedBox(height: 12),
          _label('Nota del profesional (visible para el paciente)'),
          const SizedBox(height: 6),
          _tf(_observCtrl, 'Recomendaciones, seguimiento, próxima cita...', 3),
        ])),
        const SizedBox(height: 14),
          ...
          child: Column(children: [
            InkWell(
              ...
              child: Row(children: [
                ...
                const Expanded(child: Text('Signos Vitales (opcional)',
                    style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold,
                        color: Color(0xFF1A1A7A)))),
                Icon(_showSignos ? Icons.expand_less : Icons.expand_more,
                    color: Colors.grey),
              ]),
            ),
            if (_showSignos) ...[
              ...
              narrow
                  ? Column(children: [
                      Row(children: [
                        Expanded(child: _vital(_pSisCtrl, 'Presión Sistólica', 'mmHg')),
                        const SizedBox(width: 10),
                        Expanded(child: _vital(_pDiaCtrl, 'Presión Diastólica', 'mmHg')),
                      ]),
                      const SizedBox(height: 10),
                      Row(children: [
                        Expanded(child: _vital(_fcCtrl, 'Frec. Cardíaca', 'lpm')),
                        const SizedBox(width: 10),
                        Expanded(child: _vital(_tempCtrl, 'Temperatura', '°C')),
                      ]),
                      const SizedBox(height: 10),
                      Row(children: [
                        Expanded(child: _vital(_pesoCtrl, 'Peso', 'kg')),
                        const SizedBox(width: 10),
                        Expanded(child: _vital(_tallaCtrl, 'Talla', 'cm')),
                        const SizedBox(width: 10),
          ...
          child: ElevatedButton(
            ...
            child: _isSubmitting
                ...
                : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    SizedBox(width: 8),
                    Text('Guardar Evolución', style: TextStyle(
  ...
  Widget _buildPatientHeader(Map<String, dynamic> p) {
    final name     = '${p['name'] ?? ''} ${p['last_name'] ?? ''}'.trim();
    final expNum   = 'WM-${DateTime.now().year}-${(p['id'] ?? 0).toString().padLeft(4, '0')}';
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Column(children: [
            ...
            ElevatedButton(
              ...
              child: const Text('Signos Vitales Recientes',
                  style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
            ),
          ]),
        ]),
      ...
      Row(mainAxisSize: MainAxisSize.min, children: [
        Container(
          ...
          decoration: BoxDecoration(
              ...
              shape: BoxShape.circle),
          child: Icon(icon,
              color: color ?? const Color(0xFF4F46E5), size: 14),
        ),
  ...
  Widget _infoChipAlert(IconData icon, String label, String value) =>
      Row(mainAxisSize: MainAxisSize.min, children: [
        Container(
          padding: const EdgeInsets.all(6),
          decoration: BoxDecoration(
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: TextStyle(
              fontSize: 9, color: Colors.red[300],
              fontWeight: FontWeight.bold, letterSpacing: 0.5)),
  ...
  // ── Línea de tiempo lateral
  Widget _buildTimeline() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Padding(
      padding: const EdgeInsets.fromLTRB(16, 16, 16, 0),
      child: Row(children: [
        const Icon(Icons.timeline, color: Color(0xFF4F46E5), size: 18),
        const SizedBox(width: 8),
        const Expanded(child: Text('Línea de Tiempo Clínica',
            style: TextStyle(fontWeight: FontWeight.bold,
                color: Color(0xFF1A1A7A), fontSize: 15))),
    ...
    if (_patientId == null)
      ...
      Padding(padding: const EdgeInsets.all(20),
          child: Center(child: Column(children: [
            ...
            const SizedBox(height: 8),
            Text('Sin consultas previas registradas.',
                textAlign: TextAlign.center,
                style: TextStyle(color: Colors.grey[400], fontSize: 12)),
          ])))
    else
    ...
    if (_timeline.isNotEmpty)
      Padding(
        ...
        child: Center(child: TextButton(
          onPressed: () {},
          child: const Text('CARGAR CONSULTAS ANTERIORES',
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(
              ...
              Text(name, style: const TextStyle(fontWeight: FontWeight.w600,
                  fontSize: 13, color: Color(0xFF1A1A7A))),
              Text('Doc: $doc', style: TextStyle(fontSize: 11, color: Colors.grey[500])),
            ])),
  ...
  // ── UI Helpers
  Widget _section({required IconData icon, required String title,
  ...
  void dispose() {
    ...
    _cie10Ctrl.dispose(); _diagNombreCtrl.dispose(); _diagDescCtrl.dispose();
    _tratamientoCtrl.dispose(); _observCtrl.dispose();
    _pSisCtrl.dispose(); _pDiaCtrl.dispose(); _fcCtrl.dispose();
    _tempCtrl.dispose(); _pesoCtrl.dispose(); _tallaCtrl.dispose();
    _spo2Ctrl.dispose();
    super.dispose();
  }
}
  ...
  Widget build(BuildContext context) => IntrinsicHeight(
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(record.formattedDate, style: TextStyle(
              fontSize: 11, fontWeight: FontWeight.bold,
              color: isFirst ? const Color(0xFF4F46E5) : Colors.grey[500],
              letterSpacing: 0.3)),
          const SizedBox(height: 3),
          Text(record.diagnosticoNombre ?? record.motivoConsulta,
              style: const TextStyle(fontWeight: FontWeight.bold,
                  fontSize: 13, color: Color(0xFF1A1A7A)),
              maxLines: 2, overflow: TextOverflow.ellipsis),
          const SizedBox(height: 2),
          Text('Dr. ${record.doctorFullName}',

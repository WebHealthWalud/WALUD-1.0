    ...
    if (mounted) {
      setState(() {
        ...
        if (summaryR['success']) {
          ...
          _totalPendiente =
              double.tryParse(data['total_pendiente']?.toString() ?? '0') ?? 0;
          _totalCompletado =
              double.tryParse(data['total_completado']?.toString() ?? '0') ?? 0;
        }
      });
    }
  }
    ...
    if (mounted) {
      if (r['success'] == true) {
        await showDialog(
          context: context,
          builder: (_) =>
              _PaymentSuccessDialog(payment: payment, metodoPago: metodoPago),
        );
        await _loadData();
      } else {
              ...
              child: Column(
                children: [
                  ...
                  _filteredCompleted.isEmpty
                      ...
                      : Column(
                          children: _filteredCompleted
                              .asMap()
                              .entries
                              .map((e) => _historialRow(e.value, e.key.isOdd))
                              .toList(),
                        ),
                ],
              ),
...
class _PaymentModalState extends State<_PaymentModal> {
  ...
  final _fmt = NumberFormat('#,##0.00', 'es');
  final _cardNumberCtrl = TextEditingController(text: '0000');
  final _cardNameCtrl = TextEditingController(text: 'walud');
  final _cardExpCtrl = TextEditingController(text: '00/00');
  final _cardCvvCtrl = TextEditingController(text: '000');
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    '\$${_fmt.format(widget.payment.monto)}',
                    style: const TextStyle(
                      fontSize: 22,
                      fontWeight: FontWeight.w900,
                      color: Color(0xFF4F46E5),
              ...
              child: ElevatedButton(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text(
                      'Pagar \$${_fmt.format(widget.payment.monto)}',
                      style: const TextStyle(
                        fontSize: 15,
                        fontWeight: FontWeight.bold,
                      ),
            ...
            if (payment.metodoPago != null)
              Row(
                ...
                children: [
                  ...
                  Text(
                    _metodoPagoLabel(payment.metodoPago!.name),
                    style: const TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.w500,
                    ),

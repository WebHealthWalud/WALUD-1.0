...
const double _kPatBreakpoint = 700;
...
class _PatientsScreenState extends State<PatientsScreen> {
  ...
  Future<void> _search() async {
    ...
    final r = await UserService.searchPatientByDocument(_docCtrl.text.trim(), _tipoDoc);
    setState(() {
      _isSearching = false;
      _searched    = true;
  ...
  Widget build(BuildContext context) {
    final isNarrow = MediaQuery.of(context).size.width < _kPatBreakpoint;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            // En pantallas pequeñas apila los campos en columna
            if (isNarrow) ...[
              DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(value: 'cedula_ciudadania',
                      child: Text('CC - Cédula Ciudadanía', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'tarjeta_identidad',
                      child: Text('TI - Tarjeta Identidad', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'registro_civil',
                      child: Text('RC - Registro Civil', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'cedula_extranjeria',
                      child: Text('CE - Cédula Extranjería', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'carne_diplomatico',
                      child: Text('CD - Carné Diplomático', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'pasaporte',
                      child: Text('PA - Pasaporte', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'permiso_especial_permanencia',
                      child: Text('PEP - Permiso Especial', style: TextStyle(fontSize: 12))),
                  DropdownMenuItem(value: 'permiso_proteccion_temporal',
                      child: Text('PPT - Prot. Temporal', style: TextStyle(fontSize: 12))),
                ],
      ...
      child: Column(children: [
          ...
          child: Row(children: [
            Container(
              ...
              child: p.hasPhoto == true
                  ? ClipOval(child: Image.network(p.fullPhotoUrl!, fit: BoxFit.cover,
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 6),
                Text('$edad años', style: const TextStyle(color: Colors.white70, fontSize: 13)),
              ]),
            ])),
            Container(
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: _statCard('TIPO DE SANGRE', p.tipoSangre ?? '—',
                const Color(0xFFEF4444), Icons.water_drop_outlined)),
              const SizedBox(width: 12),
              Expanded(child: _statCard('GÉNERO', _generoLabel(p.genero),
                const Color(0xFF4F46E5), Icons.person_outline)),
              const SizedBox(width: 12),
              Expanded(child: _statCard('TELÉFONO', p.phone ?? '—',
                const Color(0xFF06B6D4), Icons.phone_outlined)),
            ]),
            if (p.alergias != null && p.alergias!.isNotEmpty) ...[
              ...
              Wrap(spacing: 8, runSpacing: 6,
                children: p.alergias!.split(',').map((a) => Container(
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                  decoration: BoxDecoration(
  ...
  String _tipoDocLabel(String tipo) {
    const map = {
      'cedula_ciudadania': 'CC', 'tarjeta_identidad': 'TI',
      'registro_civil': 'RC', 'cedula_extranjeria': 'CE',
      'carne_diplomatico': 'CD', 'pasaporte': 'PA',
      'permiso_especial_permanencia': 'PEP', 'permiso_proteccion_temporal': 'PPT',
    };
  ...
  String _generoLabel(String? genero) {
    const map = {
      ...
      'otro': 'Otro', 'prefiero_no_decir': 'No especificado',
    };
    return map[genero] ?? '—';
  }
